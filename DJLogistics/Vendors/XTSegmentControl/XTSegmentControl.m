...
#define XTSegmentControlHspace (50)
...
//                [self addSubview:_titleIconView];
            ...
            default:
            {
                _titleLabel = ({
                    ...
                    label.textAlignment = NSTextAlignmentCenter;
                    label.text = title;
                    label.textColor = COLOR_FontText;
                    label.backgroundColor = [UIColor clearColor];
                    label;
                });
...
//                [_titleLabel setTextColor:(selected ? kColorBrandGreen:kColor222)];
        ...
        {
            if (_titleLabel) {
                [_titleLabel setTextColor:(selected ? COLOR_BlueDark:COLOR_FontText)];
            }
        }
            break;
    }
...
- (void)initItemsWithTitleArray:(NSArray *)titleArray withIcon:(BOOL)isIcon
{
    ...
    if ([obj isKindOfClass:[NSString class]]) {
        for (int i = 0; i < titleArray.count; i++) {
            ...
            float width = self.bounds.size.width/titleArray.count;
    ...
    if (isIcon) {
        ...
        for (int i=1; i<_itemFrames.count; i++) {
            ...
            UIView *lineView  = [[UIView alloc] initWithFrame:CGRectMake(
                                                                         ...
                                                                         1,
                                                                         14)];
            lineView.backgroundColor =  COLOR_BlueDark;
...
- (void)addRedLine
{
    if (!_lineView) {
        ...
        bottomLineView.backgroundColor = COLOR_FontText;
...
{
    if (_contentView.contentSize.width <= UI_SCREEN_WIDTH) {
        return;
    }
